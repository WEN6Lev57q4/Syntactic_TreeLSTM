...
if __name__ == '__main__':
    print('=' * 80)
    print('Preprocessing mnli_1.0 dataset')
    print('=' * 80)
    base_dir = os.path.dirname(os.path.realpath(__file__))
    dst_dir = os.path.join(base_dir, 'word_sequence')
    snli_dir = os.path.join(base_dir, 'mnli/multinli_1.0')
    make_dirs([dst_dir])

    build_sequence(os.path.join(snli_dir, 'multinli_1.0_dev_matched.txt'), dst_dir)
    build_sequence(os.path.join(snli_dir, 'multinli_1.0_dev_mismatched.txt'), dst_dir)
    build_sequence(os.path.join(snli_dir, 'multinli_1.0_train.txt'), dst_dir)
    ...
    build_dictionary([os.path.join(dst_dir, 'premise_multinli_1.0_train.txt'), 
                      ...
                      os.path.join(dst_dir, 'mnli_vocab_cased.pkl'))
